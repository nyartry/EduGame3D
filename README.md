...
ビルド時に、利用条件の`LICENSE`と外部ライブラリ等の`THIRD_PARTY_NOTICES.txt`をEXEの出力先へ配置します。配布が許可される構成でEXEを渡す際は、この2ファイルも同梱してください。詳細は[素材・ライブラリの利用条件](docs/rights-inventory.md#exeを配布するとき)を参照してください。
