...
`AnimationEvents` はエディターと実行系で同じ `edugame3d-animation-events-v1` の読込・保存処理を使います。旧schema `open-campus-animation-events-v1` も読み込めます。保存時には新schemaへ統一します。schema、必須の時刻/アニメーション名/type、有限かつ非負の時刻、JSON 構文を検証し、無効データは理由付きで失敗します。実行系の文字列に編集バッファの制限はありません。エディターに収まらない文字列は切り捨てず、読込エラーにします。
