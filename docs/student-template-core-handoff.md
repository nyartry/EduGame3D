...
方針更新（2026-09-14）: 現在のプロジェクト名は **EduGame3D（授業用3Dゲームフレームワーク）** です。日本国内の教育機関での授業利用を想定し、権利者へのSARTRAS補償金の分配を目指して公開準備を進めます。制度上の対象・権利関係は確認中です。以下の過去の目的・調査結果は作成時点の記録として残し、現在の方針は [README](../README.md) と [SARTRAS準備調査](sartras-readiness-audit.md) を優先してください。汎用3Dフレームワークとサンプルゲームの責任分担は維持します。
...
| 項目 | 内容 |
| --- | --- |
| プロジェクト | `EduGame3D`（名称は2026-09-14に更新。以下の調査基準は変更前の記録） |
...
| 2026-09-11 | R2 終了順・GPU資源寿命 | R1のユーザーコミット`a52a3ec`を確認後に着手。ゲーム／エディターとも資源所有者の破棄前にGPUを停止。記録中フレームの中断、有限待機、削除確認後の解放、解放例外の封じ込めを追加 | 検証対象は`a52a3ec`上の今回の作業ツリー（ユーザーコミット待ち）。Debug／Release統合検証成功、ネイティブ各77件。実アプリの通常終了と故障注入も確認。下記のR2記録と[終了契約](render-shutdown.md)参照 |
| 2026-09-11 | R3 ウィンドウ寸法変更 | R2のユーザーコミット`108b5f7`を確認後に着手。描画前に実クライアント寸法をrenderer・Scene・UIへ通知。カメラ、HUD、タイトル、loading/fadeを追従。0寸法の描画停止と復帰時リセットを追加 | 検証対象は`108b5f7`上の今回の作業ツリー（コミットはユーザーが行う）。Debug／Release統合検証成功、ネイティブ各87件。Debug実操作とresize例外注入を確認。下記R3記録と[寸法変更の契約](viewport-resize.md)参照 |
...
- 修正前に、遅延解放するシーンの`Unload`へ例外を注入した新規テストが`unload failure`で失敗することを確認。修正後は後続シーンの解放が続き、更新時の元の例外も維持される。診断sinkが例外を投げる場合も検証。
- `RendererLifecycleTests`を追加（CPU1・WARP6）。未初期化／途中初期化、正常フレーム送信、送信済みコピーと記録中フレームの同居、ガードと所有者の破棄順、元の例外保持、二重終了、例外・再入のある遅延解放を確認。
- GPUキューを10秒以上待たせる故障注入で、タイムアウト後にデバイス削除を確認するまで解放しないことを検証。診断sinkから追加された解放も同様。明示的なデバイス削除も検証。このPCでは削除API非対応によるスキップなし。
- `tools/validate_core.ps1`でDebug／Releaseとも成功。GameApp・AnimationEventEditor・依存ライブラリ、CLI全11種類、WARP、ネイティブ各77件、HLSL4ファイルのVS／PSを確認。ログは`x64/student-template-r2-validation.log`。
- 実際のDebug GameApp／AnimationEventEditorの2フレーム目の描画記録中に例外を一時注入。元のメッセージがエラーダイアログに表示され、閉じた後に終了コード1となることを確認。エディターはImGuiコンテキスト生成直後の例外も同様に確認。ログは`x64/r2-game-frame-fault.log`、`x64/r2-editor-frame-fault.log`、`x64/r2-editor-context-fault.log`。
- 注入したソースは元のバイト列へ戻し、両アプリを再ビルド。ゲームのタイトルとエディターのグリッド・UIを画面確認し、通常操作で終了コード0。ログは`x64/r2-GameApp-normal.log`、`x64/r2-AnimationEventEditor-normal.log`。検証用の注入は変更に含まれない。
- 制限: Releaseの実画面操作、実機GPUの物理障害、全ドライバー、削除API非対応かつ待機不能での`std::terminate`経路は未検証。GPU復元、描画後にローカル資源を直ちに破棄するような契約違反、資源退避の割当失敗の全面的な復旧は含めない。
- 次の1コミット分の候補はR3「ウィンドウ寸法変更の通知」。R3〜R5とC1〜C7は未実装。

### R3の検証記録

- `WM_SIZE`を記録し、描画開始前に`GetClientRect`の最新正寸法をrenderer、SceneManagerの順で反映。Windowsコールバック内ではGPU／UIを操作しない。ウィンドウとcallback用状態を、例外時もRAIIで管理する。
- `IScene::OnResize`を追加。有効な全シーンへ通知し、新しい候補には同期／非同期とも`Prepare`完了後・`Activate`直前に最新寸法を渡す。候補への通知失敗は原因を記録して旧シーンを保持し、再試行可能。
- カメラ比率、HUD中央／右端、画像の下端位置、loadingとfadeの矩形を更新。Titleのボタンは実寸法でレイアウトし、文字はRmlUiから取得したborder boxへ配置する。ポインターは同じクライアント座標を直接使用。小さいloading表示とTitleの文字も調整。
- 最小化／0寸法では描画・固定更新を止め、次のWindowsメッセージを待機。復帰・移動／サイズ変更後は時計と固定更新の入力バッファをリセットする。
- 回帰追加10件: Scene5件、RmlUiをGPUなしで使うUI4件、WARPの連続リサイズ1件。非同期準備中の複数変更、通知のスレッドと順序、Additive、0／同一寸法、通知失敗と再試行、描画前の最初のクリック、リスナー維持、記録中resize拒否を確認。
- `tools/validate_core.ps1`でDebug／Release成功。GameApp・AnimationEventEditor・依存ライブラリ、CLI全12種類、WARP、ネイティブ各87件（CPU77・GPU10）、HLSL4ファイルのVS／PSを検証。ログは`x64/student-template-r3-validation.log`。
- Debug実操作でTitleの最大化／復元、幅縮小、新しい位置のprobe／sample／STARTクリック、ゲームへの遷移、ゲーム画面の高さ変更、Title・Gameの最小化／復帰、Escapeによる終了コード0を確認。最小化中の2秒間のCPU時間増分はこの測定では0ms。ログは`x64/r3-game-normal.log`、`x64/r3-minimized-cpu.log`。
- 統合検証後の画面確認で、縮小したTitleのサンプル文字に欠けを見つけた。ボタン内に収まる場合は字のセルを1px以上にして描画原点をピクセル境界へ揃える表示調整を追加し、Debug／ReleaseのGameAppを再ビルド。ログは`x64/r3-final-Debug-build.log`、`x64/r3-final-Release-build.log`。
- リサイズ後に一時的に`R3 resize failure`例外を注入。GPU停止とウィンドウ破棄の後に同じ原因のFatal errorが表示され、閉じた後の終了コード1を確認。注入ソースは元のバイト列へ戻した。ログは`x64/r3-resize-fault.log`。検証用例外は最終差分に含めない。
- 最終Release版でもTitleの起動、幅の縮小、サンプルボタンの文字とクリック、正常終了を確認。ログは`x64/r3-final-Release-normal.log`。
- 制限: Releaseのゲームシーンの通し操作、DPIの異なるモニター間移動、長時間・全ドライバーの試験は未実施。Gameの左上HUDと固定サンプル図形は自動縮小せず、狭い幅では右上文字と重なる場合がある。極小画面の全コンテンツ可読性、DPI対応、フルスクリーンは今回の完了条件に含めない。
- 次の1コミット分の候補はR4「GameApp単独の配布・起動手順」。R4・R5とC1〜C7は未実装。
