...
void TitleScene::Update(float deltaTime, const Input& input)
{
	...
	if (m_acceptStartInput && input.WasAnyPressed())
	{
		m_startRequested = true;
	}
	m_acceptStartInput = true;
...
void TitleScene::RebuildBatch()
{
	...
	const float promptSize = std::clamp(width / 205.0f, 2.6f, 4.6f);
	...
	DrawCenteredText("ANY KEY / CLICK TO START", centerY + 92.0f + pulse * 4.0f, promptSize, PromptColor);
